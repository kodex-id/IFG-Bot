...
let handler = async (m, { conn, usedPrefix, owner }) => { 
    try { 
        ...
        if (global.db.data.users[m.sender].health > 79) {
            ...
            let coin = `${Math.floor(Math.random() * 100000)}`.trim() 
            let _potion = ['1','2','3']
            ...
            let str = `
Kesehatanmu❤️ mu berkurang -${health * 1} karena Kamu telah berpetualang sampai ${pickRandom(['Jepang', 'Korea', 'Bali', 'Amerika', 'Iraq', 'Arab', 'Pakistan', 'German', 'Finlandia', 'Ke bawa dunia mimpi', 'Ujung dunia', 'Mars', 'Bulan', 'Pluto', 'Matahari', 'Hatinya dia', '...'])} dan mendapatkan
*Exp:* ${exp} 
*Coin:* ${coin}
*Tiketcoin:* 1
...
`.trim()
            ...
            global.db.data.users[m.sender].money += coin * 1
            ...
            } else conn.reply(m.chat, `Kamu sudah berpetualang dan kelelahan, silahkan coba *${timers}* lagi`, m)
        } else conn.reply(m.chat, 'Minimal kesehatanmu❤️ 80 untuk bisa berpetualang, beli obat dulu dengan ketik *' + usedPrefix + 'shop buy potion <jumlah>*\ndan ketik *' + usedPrefix + 'use potion <jumlah>*', m)
    } catch (e) {
        console.log(e)
