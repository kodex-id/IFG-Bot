...
let nomors = m.sender
  ...
  if (new Date - global.db.data.users[m.sender].lastbunuhi > 3600000){
   if (10 > users[who].health) throw 'Target sudah tidak memiliki ❤️'
   if (100 > users[who].money) throw 'Target tidak memiliki apapun ☹️'
  users[who].health -= healtu * 1
  users[who].money -= dapat * 1
  users[m.sender].money += dapat * 1
  global.db.data.users[m.sender].lastbunuhi = new Date * 1
  m.reply(`Target berhasil di bunuh dan kamu mengambil Coin target sebesar\n 💵 ${dapat} Coin \nKesehatan target berkurang -${healtu} ❤️`)
