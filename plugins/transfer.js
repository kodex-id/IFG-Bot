            ...
            case 'coin':
                if (global.db.data.users[m.sender].money >= count * 1) {
                    try {
                        ...
                        conn.reply(m.chat, `Berhasil mentransfer sebesar ${count} Coin`.trim(), m)
                    } catch (e) {
                        global.db.data.users[m.sender].money += count * 1
                ...
                } else conn.reply(m.chat, `Coinmu tidak mencukupi untuk mentransfer ${count} Coin`.trim(), m)
                break 
             case 'exp':
             ...
             case 'limit':
                ...
                break
            default:
                return conn.reply(m.chat, `Gunakan format ${usedPrefix}transfer <type> <jumlah> <@tag>\ncontoh penggunaan: *${usedPrefix}transfer coin 100 @tag*\n\n*List yang bisa di transfer*\nCoin\nExp\nLimit`.trim(), m)
        }
    } catch (e) {
