...
${isSurrender ? '' : `Setiap jawaban yang benar mendapat uang Rp. ${room.winScore.toLocaleString()}`}
    `.trim()
    const msg = await this.sendButton(m.chat, caption, wm, null, [[`${(isWin || isSurrender) ? 'Main lagi' : 'Menyerah'}`, `${(isWin || isSurrender) ? '.family100' : 'nyerah'}`]], null, {
