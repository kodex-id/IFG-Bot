...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  ...
  if(isNaN(text)) {
   ...
   if(!text && !m.quoted) return conn.reply(m.chat, `*❏ Silahkan Baca petunjuk dibawah ini:*\n\n• \`\`\`\Tag user:\`\`\`\ contoh *${usedPrefix}allprofile @Tag*\n\n• \`\`\`\Tambahkan nomor:\`\`\`\ contoh *${usedPrefix}profile 60142871298*\n\n• \`\`\`\Cek Profilemu Sendiri:\`\`\`\ Balas pesanmu sendiri dan ketik *${usedPrefix}allprofile*\n\n• \`\`\`\Cek profile orang lain:\`\`\`\Balas pesan orang lain dan ketik *${usedPrefix}allprofile*`, m)
    if(isNaN(number)) return conn.reply(m.chat, `*❏ Silahkan Baca petunjuk dibawah ini:*\n\n• \`\`\`\Tag user:\`\`\`\ contoh *${usedPrefix}allprofile @Tag*\n\n• \`\`\`\Tambahkan nomor:\`\`\`\ contoh *${usedPrefix}allprofile 60142871298*\n\n• \`\`\`\Cek Profilemu Sendiri:\`\`\`\ Balas pesanmu sendiri dan ketik *${usedPrefix}allprofile*\n\n• \`\`\`\Cek profile orang lain:\`\`\`\Balas pesan orang lain dan ketik *${usedPrefix}allprofile*`, m)
 ...
 let pp = './src/avatar_contact.png'
  try {
   ...
   // let buffer = await getBuffer(pp)
    ...
    let str = `
Nama: ${username} ${registered ? '(' + name + ') ': ''}(@${who.split`@`[0]})${about ? '\nTentang: ' + about : ''}
Status: ${pasangan ? jodoh : 'Jomblo' }
Nomor: ${PhoneNumber('+' + who.replace('@s.whatsapp.net', '')).getNumber('international')}
Link: https://wa.me/${who.split`@`[0]}${registered ? '\nAge: ' + age : ''}
Terdaftar: ${registered ? 'Ya (' + new Date(regTime) + ')': 'Tidak'}
...
Role: *${role}*
Limit: *${limit}*
Exp: TOTAL ${exp} (${exp - min} / ${xp}) [${math <= 0 ? `ketik *${usedPrefix}levelup* untuk naik level` : `${math} XP lagi untuk naik level`}]
💵 Coin: *${money}*
❤️ Kesehatan: *${health}*
